...
const GameBox: React.FC<input> = ({title,chars,onExit,gameStatus,setGameStatus}) => {
        ...
        <>
          ...
          {firstGame ? (
            ...
            <div className="border relative px-4 bg-black flex mt-4 justify-center items-center space-x-1">
              ...
              <div className={`${result=="WIN"?"text-green-600":"text-red-600"} text-sm`}>{result}</div>
              <div className="mx-1 text-yellow-700 text-[40%]">({prevWord})</div>
            </div>
          )}
        </>
      )}
